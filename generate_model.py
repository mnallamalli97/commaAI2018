...
def preprocess(path_to_video):
    ...
    while success:
        ...
        cv2.imwrite("cleaned_data/frame %d.jpg" % count, img)  # save frame as JPEG file
...
def optical_flow_cleaned_frames(cleaned_frames, true_speeds):
    count = 0
    optical_flow_frames = []
    optical_flow_speeds = []
    ...
    for i in range(len(cleaned_frames[:-1])):
        ...
        cv2.imwrite("optical_flow_frames/frame%d.jpg" % count, img)  # save frame as JPEG file
        optical_flow_frames.append(img)
        count+=1

    return optical_flow_frames, optical_flow_speeds



'''this is the neural net part'''

def create_model_to_train(optical_flow_frames, optical_flow_speeds, batch_size=32):
    ...
    return frame_batch, label_batch
...
def train_model(model, x, y):
    ...
    model.fit(x, y, epochs=150, batch_size=32, validation_split=.15)
    # x[i] = x[i].reshape((1,) + x[i].shape)
    scores = model.evaluate(x, y, verbose=1)
    print("score: ",scores)
    model.save('modelv2.h5')  # creates a HDF5 file 'my_model.h5'

def main():
    # correct_speed = np.loadtxt(fname="data/train_original.txt")
    # cleaned_frames = preprocess('data/train_original.mp4')
    # cleaned_frames, cleaned_speeds = optical_flow_cleaned_frames(cleaned_frames, correct_speed)
    # print("finished with preprocess step.")
    # print("generating model...")
    # X,Y = create_model_to_train(cleaned_frames, cleaned_speeds)
    # #X is frame, Y is speed
    # print("creating results.txt file...")
    # f = open('results.txt', 'w+')
    # import sys
    # sys.stdout = f
    print("loading frames pickle")
    with open("frames.pkl", 'rb') as handle:
        X_train = np.array(pkl.load(handle))
    print("loading speeds pickle")
    with open("speeds.pkl", 'rb') as handle:
        Y_train = np.array(pkl.load(handle))

    print("training")
    train_model(mehar_model(), X_train, Y_train)
